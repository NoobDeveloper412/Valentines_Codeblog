...
//     `content/${context.query.category}/${context.query.slug}.md`,
//     "utf-8"
//   );
...
export const getStaticPaths = async () => {
  const learnDirectory = path.join("content");

  const topicDirectories = fs.readdirSync(learnDirectory);

  const allPaths = [];

  topicDirectories.forEach((category) => {
    const topicDirectory = path.join(learnDirectory, category);
...
export const getStaticProps = async ({ params: { category, slug } }) => {
  const filename = fs.readFileSync(
    path.join("content", category, slug + ".md")
  );
  const { data: frontmatter, content } = matter(filename);
